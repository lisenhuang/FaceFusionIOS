...
//  graph itself cannot run here, but every number on either side of it can:
//  the input packing (channels-last BGR at 1/255) and the output mapping
//  (clip, resize, σ5 blur, then `(clip(0.5...1) − 0.5) × 2`) are ported from
//  FaceFusion 3.0's `create_occlusion_mask` and must not drift from it.
//
...
struct FaceOccluderTests {
    ...
    @Test func occlusionCarvesTheCombinedMask() {
        ...
        let size = FaceOccluder.inputSize
        let output = FloatTensor(shape: [1, size, size, 1], repeating: 1)
        for y in 96 ..< 160 {
            for x in 96 ..< 160 {
