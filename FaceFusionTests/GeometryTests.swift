...
//
//  These are the numbers that decide whether a swapped face lands where the
//  original one was. Nothing crashes when they drift — the output just quietly
//  gets worse — so they are asserted rather than eyeballed.
//
...
struct MaskTests {
    ...
    /// The 512px enhancer mask needs a ~150-tap separable Gaussian, which is
    /// far too expensive to rebuild per frame — so it is cached, and the cache
    /// has to hand back the same mask rather than a fresh empty one.
    @Test func maskIsCachedByParameters() {
        let first = FaceMasker.boxMask(size: 512, blur: 0.3)
        let second = FaceMasker.boxMask(size: 512, blur: 0.3)
        #expect(first.values == second.values)
        #expect(first.values.contains { $0 > 0.99 }, "a cached mask must not be blank")
    }
}
