...
//  they run on any simulator, unlike anything that needs half a gigabyte of
//  weights on disk.
//
...
/// A unit vector that leans on one axis, perturbed by `jitter` of deterministic
/// noise. Two identities built from the same axis are close; two built from
/// different axes are far — which is the only property the matcher depends on.
///
/// The noise is seeded on the **axis as well as the seed**, and comes from an
/// ordinary LCG mapped onto `[-jitter, jitter)`. Both details are load-bearing,
/// and the version this was ported from got both wrong:
///
///  * Its recurrence was `x -> (x * 1.37).truncatingRemainder(dividingBy: 2) - 1`.
///    `truncatingRemainder` keeps the sign of its operand, so a negative state
///    maps into `(-2, 0]` and the `- 1` pushes it to `(-3, -1]`. Measured over
///    512 steps that lands in `[-2.994, 0.370]` with a mean of `-2.158` — a
///    large negative DC offset, not a small zero-mean perturbation. At
///    `jitter: 0.05` the "noise" was bigger than the signal it perturbed, and a
///    face was 0.68 away from itself.
///  * It was driven by `seed` alone, so two *different* people handed the same
///    seed received byte-identical noise. Three distinct axes at `jitter: 0.04`
///    came out 0.198 apart — well inside the clusterer's 0.5 threshold — so the
///    test that checks different people stay separate was comparing noise
///    against itself and merging them.
///
/// Both faults reproduce in the macOS project this was ported from, where the
/// same two tests fail. They are fixed here rather than carried across.
private func identity(axis: Int, jitter: Float = 0, seed: Int = 0) -> FaceIdentity {
    var values = [Float](repeating: 0, count: 512)
    values[axis % 512] = 1
    if jitter > 0 {
        // Numerical Recipes' LCG constants. Deterministic on every platform,
        // which is what a fixture needs; the statistical quality of the stream
        // is irrelevant at 512 samples.
        var state = UInt32(truncatingIfNeeded: axis &* 2_654_435_761 &+ seed &+ 1)
        for index in 0 ..< 512 {
            state = state &* 1_664_525 &+ 1_013_904_223
            // Top 24 bits, so the low-order bits — the worst part of an LCG —
            // are discarded rather than shaping the noise.
            let unit = Float(state >> 8) / Float(1 << 24)     // [0, 1)
            values[index] += jitter * (unit * 2 - 1)          // [-jitter, jitter)
        }
    }
...
struct FaceClusteringTests {
    ...
    @Test func mergingPullsTheIdentityTowardTheAverage() {
        var clusterer = FaceClusterer()
        // Far enough out to be visibly the worst look at this person (0.35 from
        // a clean vector) but still inside the 0.5 threshold, because the point
        // of the test is that a bad sighting gets *averaged away* rather than
        // that it gets rejected as somebody else.
        let odd = identity(axis: 6, jitter: 0.10, seed: 1)
...
struct FaceSelectionTests {
    ...
    /// Nothing crosses a process boundary any more, but the selection is still
    /// `Codable` because it is what the settings store persists — and a dropped
    /// generation would be read as a stale set and fail the next export.
