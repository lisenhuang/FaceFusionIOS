...
//
//  The engine sees one frame at a time and has no access to the file — decoding
//  belongs to the app. So the scan lives here: sample frames across the
//  duration, ask the engine for the faces and identities in each, and group
//  those identities into people.
//
...
enum FaceScanner {
    ...
    static func scan(photo buffer: CVPixelBuffer,
        ...
        let analysis = try await engine.analyzeFaces(buffer, options: options)
    ...
    static func scan(video url: URL,
        ...
        for time in times {
            ...
            // Everything above runs on the main actor, and a sample that finds
            // no faces can complete without ever suspending — which on a phone
            // means the progress bar sits frozen through a run of them and then
            // jumps. One yield per sample gives SwiftUI a turn to draw. It is
            // not a throttle: the engine call is orders of magnitude longer
            // than the hop.
            await Task.yield()
        }
