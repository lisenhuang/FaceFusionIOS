...
//  device, a rate-limited lookup, a malformed answer and an app that is already
//  current all produce exactly the same result: nothing on screen, nothing
//  logged above debug. The only visible outcome is a version genuinely newer
...
//  outcomes `check()` flattens into `nil` apart — and the distinction that
//  matters most is between "the store says you are current" and "the store did
//  not answer". Reporting the second as the first is the one failure mode a
//  manual check cannot have: it would tell someone they are up to date on the
//  strength of a dropped connection.
//
//  This is the second network request the app makes — `Downloader` makes the
//  first — and the only one that is not about models. It sends the app's own
//  App Store id and nothing else: no device identifier, no install identifier,
...
enum UpdateChecker {
    ...
    enum Outcome: Equatable, Sendable {
        ...
        /// installed one, because a TestFlight or locally built copy is ahead
        /// of the listing and saying so is more use than pretending otherwise.
        case current(latest: String)

        /// The store answered, and it is ahead of us.
        case available(Update)

        /// No usable answer: offline, rate-limited, malformed, or no listing
        /// for this platform. Deliberately not split further — none of the
        /// causes change what the user can do about it, and none of them are
    ...
    static func fetch() async -> Outcome {
        ...
        switch outcome {
        case .unavailable:
            EngineLog.app.debug("Update check: no answer from the store")
        case .current:
            EngineLog.app.debug("Update check: \(installed, privacy: .public) is current")
        case .available(let update):
            EngineLog.app.notice("Update available: \(update.version, privacy: .public)")
        }
        return outcome
    }
    ...
    private static func lookup(country: String?) async -> Update? {
        ...
        do {
            ...
            EngineLog.app.debug("Update check failed: \(error.localizedDescription, privacy: .public)")
            return nil
        }
    }
