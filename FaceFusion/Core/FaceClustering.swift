...
//  FaceFusion
//
//  Turning "faces seen in the sampled frames" into "the people in this video".
//
//  Pure arithmetic over identity vectors, with no image or media types in
//  sight, which is what lets it be reasoned about — and tested — without a
//  single model on disk.
//
...
struct FaceClusterer {
    ...
    struct Person: Sendable, Identifiable {
        var id: Int
    ...
    @discardableResult
    mutating func add(_ identity: FaceIdentity,
                      at time: Double,
                      score: Double,
