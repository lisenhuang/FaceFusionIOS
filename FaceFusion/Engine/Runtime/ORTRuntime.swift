...
//
//  ONNX Runtime is linked into the app itself. iOS has no XPC for third-party
//  code, so unlike the Mac version there is no helper process to keep the
//  560 MB of weights out of; the models live here, and inference is dispatched
//  to the Core ML execution provider, which hands work to the Neural Engine or
//  the GPU. Whatever Core ML cannot absorb falls back to ORT's own CPU kernels.
//
//  The one structural change from the Mac version is where a tensor's bytes
//  live, and everything downstream depends on it. There, a `FloatTensor` was a
//  `[Float]` that got memcpy'd into a fresh `NSMutableData` on every `run`.
//  Here the bytes are allocated once, out of a shared-storage `MTLBuffer`
//  whenever Metal is available — so the compute kernel that fills a tensor and
//  ONNX Runtime that reads it address the same physical pages. On Apple
//  Silicon there is no upload, no download and no copy anywhere on the hot
//  path, only a pointer handed from one consumer to the next.
//
...
final class ORTRuntime {
    ...
    private static let sharedEnv: Result<ORTEnv, Error> = {
        // The Mac build raised this to `.verbose` behind a `--profile` argv
        // flag. There is no argv on iOS; the equivalent switch is
        // `EngineTuning.profileComputePlan`, which raises the *session* log
        // level instead, so the environment stays quiet.
    ...
    /// model behind it. Used when the system asks for memory back: losing the
    /// enhancer costs sharpness, whereas being jetsammed costs the render.
    ...
    func load(_ id: ModelID, path: String, compute: ComputePolicy,
        ...
        if compute != .cpu, ORTIsCoreMLExecutionProviderAvailable() {
            ...
            var providerOptions: [String: String] = [
                ...
                // EP absorb regions it would otherwise leave to the CPU, which
                // the Mac measured as a free 35% — this flag was originally
                // off, and that was simply a mistake.
            ...
            // On the Mac, pinning these graphs to `CPUAndNeuralEngine` was 17×
            // *slower* than letting Core ML choose — they are convolutional
