...
//  weight blobs are skipped over without ever being paged in — which matters
//  rather more on a phone than it did on a Mac, since paging them all in would
//  be 278 MB of footprint bought for 1 MB of matrix.
//
...
enum OnnxInitializerReader {
    ...
    private static func parseTensor(buf: UnsafeRawBufferPointer,
        ...
        guard let fileBase = buf.baseAddress else {
            throw makeEngineNSError(.modelLoadFailed, underlying: "empty model file")
        }
        ...
        if let rawData {
            ...
            case 1:  // FLOAT32
                ...
                floats = [Float](unsafeUninitializedCapacity: count) { dst, initialized in
                    ...
                    guard let destination = dst.baseAddress else { initialized = 0; return }
                    memcpy(destination, fileBase + rawData.lowerBound, expected)
                    initialized = count
                }
            case 10: // FLOAT16
                ...
                halves.withUnsafeMutableBytes {
                    ...
                    _ = memcpy(destination, fileBase + rawData.lowerBound, expected)
                }
            ...
            floats = [Float](unsafeUninitializedCapacity: n) { dst, initialized in
                guard let destination = dst.baseAddress else { initialized = 0; return }
                memcpy(destination, fileBase + packedFloats.lowerBound,
