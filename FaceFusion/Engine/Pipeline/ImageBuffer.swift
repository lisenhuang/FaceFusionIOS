...
//
//  Everything works on 32-bit BGRA, which is what AVFoundation decodes into and
//  what CoreVideo carries. Byte order in memory is B, G, R, A at ascending
//  addresses, so "channel 0" is blue. The reference Python pipeline is built on
//  OpenCV and is likewise BGR-first, which keeps the channel bookkeeping below
//  identical to it.
//
...
//  disagree, the CPU is right. A device without a usable Metal library — or a
//  frame the GPU cannot address, which is any borrowed buffer that failed the
//  page-alignment test in `wrapping` — simply falls through and still works.
//
...
/// A mutable 8-bit BGRA image. Either owns its storage or borrows a
/// `CVPixelBuffer`'s, in which case the caller keeps that buffer locked.
final class BGRAImage {
    ...
    /// As above, but with a GPU view onto the borrowed bytes. Only `wrapping`
    /// builds one of these, because only it knows the buffer stays locked and
    /// alive for exactly as long as the image does.
...
// MARK: - Pixel buffer bridging

extension BGRAImage {

    /// Runs `body` with an image view onto a locked `CVPixelBuffer`.
    ///
    /// The frames come from AVFoundation, so they are already backed by an
    /// `IOSurface`. When that surface's base address is page-aligned and its
    /// allocation covers the plane, Metal can be handed the same pages with
    /// `makeBuffer(bytesNoCopy:)` and a decoded frame reaches the GPU without
    /// being copied at all. When the alignment does not hold — and it is not
    /// guaranteed by anything — the image simply has no `mtlBuffer` and every
    /// operation on it takes the CPU path. That is a slower frame, never a
    /// failed one.
    ///
    /// The returned image must not escape `body`: the pixel buffer is unlocked
    /// on the way out and the Metal view over it dies with it.
    static func wrapping<T>(_ pixelBuffer: CVPixelBuffer,
                            readOnly: Bool,
        ...
        guard let base = CVPixelBufferGetBaseAddress(pixelBuffer) else {
            // A locked 32BGRA buffer always has one. If somehow it does not,
            // handing `body` a correctly sized scratch image loses the frame,
            // which beats trapping in the middle of an export.
            EngineLog.metal.error("pixel buffer had no base address; frame lost")
            return try body(BGRAImage(width: max(1, width), height: max(1, height)))
        }

        let shared = gpuView(over: base, in: pixelBuffer, byteCount: rowBytes * height)
    ...
    private static func gpuView(over base: UnsafeMutableRawPointer,
        ...
        // and `IOSurfaceGetAllocSize` reports the surface's size, not its
        // mapping: a 1080p BGRA surface comes back as the plane plus 64 bytes,
        // which is not page-rounded, while the mapping behind it always is.
        // Rounding the length up is therefore safe, and testing the *rounded*
        // length against the alloc size would reject nearly every real frame.
        guard IOSurfaceGetAllocSize(surface) >= byteCount else { return nil }
        let length = (byteCount + pageSize - 1) / pageSize * pageSize

        // No deallocator: the surface owns these pages and outlives the buffer,
        // which lives only for the duration of `wrapping`.
